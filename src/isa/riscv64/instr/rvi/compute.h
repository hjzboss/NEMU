...
def_EHelper(add) {
  // 源头是/home/hjz/com_core/NEMU/src/engine/interpreter/rtl-basic.h
...
def_EHelper(crc32) {
  *ddest = Crc32Table[(*dsrc2 ^ *dsrc1) & 0xff] ^ (*dsrc2 >> 8);
}

def_EHelper(crc32w) {
  *ddest = Crc32Table[(*dsrc2 ^ *dsrc1) & 0xff] ^ ((unsigned)*dsrc2 >> 8);
}

def_EHelper(mulhash32) {
  *ddest = (int32_t)((uint32_t)(*dsrc1 * *dsrc2) >> (31 - BITS(*dsrc3, 4, 0)) >> 1);
  //printf("src1=%lx, src2=%lx, src3=%lx, dest=%lx\n", *dsrc1, *dsrc2, *dsrc3, *ddest);
}

def_EHelper(mulhash64) {
  *ddest = (*dsrc1 * *dsrc2) >> (63 - BITS(*dsrc3, 5, 0)) >> 1;
}
