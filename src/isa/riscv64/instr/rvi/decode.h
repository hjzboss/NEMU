...
// 获取寄存器的值，保存至id_src->preg中
static inline def_DHelper(R4) {
  decode_op_r(s, id_src1, s->isa.instr.r4.rs1, true);
  decode_op_r(s, id_src2, s->isa.instr.r4.rs2, true);
  decode_op_r(s, id_src3, s->isa.instr.r4.rs3, true);
  decode_op_r(s, id_dest, s->isa.instr.r4.rd, false);
}
...
def_THelper(crc) {
  def_INSTR_TAB("0100000 ????? ????? ??? ????? ????? ??", crc32w);
  def_INSTR_TAB("0000000 ????? ????? ??? ????? ????? ??", crc32);
  return EXEC_ID_inv;
}

def_THelper(mulhash) {
  def_INSTR_TAB("?????00 ????? ????? ??? ????? ????? ??", mulhash32);
  def_INSTR_TAB("?????01 ????? ????? ??? ????? ????? ??", mulhash64);
  return EXEC_ID_inv;
}