...
/*
  逻辑梳理：table_main用于指令的执行，其中的主要逻辑是先通过decode_R、decode_I（通过宏定义def_DHelper产生）等等函数来读取寄存器或者立即数的值，然后调用对应的table_mul、table_add，
  table_mul被映射到了exec_mul函数，进行指令的执行。

  添加新指令的步骤：
  在def_THelper(main)中添加映射
  然后再在对应的扩展下的decode.h中添加具体的映射，在exec.h中添加指令的运行方式
  在/home/hjz/com_core/NEMU/src/isa/riscv64/include/isa-all-instr.h中对新的指令进行注册
*/
def_THelper(main) {
  def_INSTR_IDTAB("??????? ????? ????? 111 ????? 1111011",  R     , crc);
  def_INSTR_IDTAB("??????? ????? ????? 010 ????? 0001011",  R4    , mulhash);
